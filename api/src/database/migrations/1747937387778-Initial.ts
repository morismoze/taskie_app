...
export class Initial1747937387778 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."task_assignment_status_enum" AS ENUM('IN_PROGRESS', 'COMPLETED', 'COMPLETED_AS_STALE', 'CLOSED')`,
    );
    await queryRunner.query(
      `CREATE TABLE "task_assignment" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP, "status" "public"."task_assignment_status_enum" NOT NULL DEFAULT 'IN_PROGRESS', "task_id" uuid, "assignee_id" uuid, CONSTRAINT "PK_bbd5007caf5731217005cccdc0d" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."goal_status_enum" AS ENUM('IN_PROGRESS', 'COMPLETED', 'COMPLETED_AS_STALE', 'CLOSED')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."workspace_user_workspace_role_enum" AS ENUM('MANAGER', 'MEMBER')`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."workspace_user_status_enum" AS ENUM('ACTIVE', 'REMOVED')`,
    );
    await queryRunner.query(
      `CREATE TABLE "workspace_user" ("id" uuid NOT NULL DEFAULT uuid_generate_v4(), "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "deleted_at" TIMESTAMP, "workspace_role" "public"."workspace_user_workspace_role_enum" NOT NULL, "status" "public"."workspace_user_status_enum" NOT NULL, "workspace_id" uuid, "user_id" uuid, "created_by_id" uuid, CONSTRAINT "UQ_workspace_user" UNIQUE ("user_id", "workspace_id"), CONSTRAINT "PK_a09cff0ab849da007d391eb9284" PRIMARY KEY ("id"))`,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_b312f71c6b1049d0455261f39d" ON "workspace_user" ("workspace_id") `,
    );
    await queryRunner.query(
      `CREATE INDEX "IDX_cb830469656d51ce772872fc9d" ON "workspace_user" ("user_id") `,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `DROP INDEX "public"."IDX_cb830469656d51ce772872fc9d"`,
    );
    await queryRunner.query(
      `DROP INDEX "public"."IDX_b312f71c6b1049d0455261f39d"`,
    );
    await queryRunner.query(`DROP TABLE "workspace_user"`);
    await queryRunner.query(`DROP TYPE "public"."workspace_user_status_enum"`);
    await queryRunner.query(
      `DROP TYPE "public"."workspace_user_workspace_role_enum"`,
    );
